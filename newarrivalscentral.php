...
$centralListingsStmt = $conn->prepare(
    "SELECT l.*, u.businessname AS seller_businessname, u.profilepic AS seller_profilepic
     ...
     WHERE COALESCE(l.out_of_stock, 0) = 0
       ...
       AND LOWER(TRIM(COALESCE(l.region, ''))) = 'central'
       AND l.created_at >= DATE_SUB(NOW(), INTERVAL 14 DAY)
     ORDER BY l.created_at DESC, l.listing_id DESC"
);

if ($centralListingsStmt) {
    $centralListingsStmt->execute();
    $centralListingsRes = $centralListingsStmt->get_result();
    while ($row = $centralListingsRes->fetch_assoc()) {
    ...
    <main class="main-one">
        <div class="container-fluid">
            <div class="container mb-3 mt-3 newarrivals-container-one">
                <h4 style="font-weight: 700">New Arrivals - <span style="color: rgb(241, 90, 36);">Central
                        Uganda</span>
                </h4>
                <hr>
                <p>Stay ahead with the latest business listings from Central Uganda. This section highlights newly
                    added
                    products and services
                    from trusted businesses across the region, including <b style="color: rgb(241, 90, 36);">Kampala,
                        Wakiso, Mukono,</b> and more.
                    Whether
                    you're sourcing in bulk, exploring
                    fresh opportunities, or expanding your supplier network, these latest arrivals are curated to
                    keep
                    you connected and informed. <i><b>Check
                            back regularly for the newest updates in wholesale trade.</b></i>
                </p>
            </div>
                        ...
                        <?php } else { ?>
                            <div class="card">
                                <div class="card-body p-2">
                                    <p class="mb-0">No Central-region video listings have been posted in the last 14 days.</p>
                                </div>
                            </div>
                        <?php } ?>
                        ...
                        <?php } else { ?>
                            <div class="card">
                                <div class="card-body p-3">
                                    <p class="mb-0">No Central-region image listings have been posted in the last 14 days.</p>
                                </div>
                            </div>
                        <?php } ?>
                        <div class="listing-load-more-wrap">
