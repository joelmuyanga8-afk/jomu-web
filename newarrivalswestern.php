...
$westernListingsStmt = $conn->prepare(
    "SELECT l.*, u.businessname AS seller_businessname, u.profilepic AS seller_profilepic
     ...
     WHERE COALESCE(l.out_of_stock, 0) = 0
       ...
       AND LOWER(TRIM(COALESCE(l.region, ''))) = 'western'
       AND l.created_at >= DATE_SUB(NOW(), INTERVAL 14 DAY)
     ORDER BY l.created_at DESC, l.listing_id DESC"
);

if ($westernListingsStmt) {
    $westernListingsStmt->execute();
    $westernListingsRes = $westernListingsStmt->get_result();
    while ($row = $westernListingsRes->fetch_assoc()) {
...
<head>
    ...
    <title>New Arrivals | Western Uganda</title>
    <meta name="descrption" content="Explore new products, suppliers, businesses and services from
    Western Uganda on JoMu. Find fresh opportunities and trusted connections.">
    ...
    <main class="main-one">
        <div class="container-fluid">
            <div class="container mb-3 mt-3 newarrivals-container-one">
                <h4 style="font-weight: 700">New Arrivals - <span style="color: rgb(241, 90, 36);">Western
                        Uganda</span>
                </h4>
                <hr>
                <p>Stay updated with the freshest listings from Western Uganda. Explore new arrivals from towns like <b style="color: rgb(241, 90, 36);">Mbarara,
                        Fort Portal, Kabale, Hoima, Bushenyi</b> and more.
                    Whether
                    you're sourcing products or offering services, this space connects you to the latest opportunities in the Western region.
                    <i><b>Check
                            in often, new deals are added regularly.</b></i>
                </p>
            </div>
                        ...
                        <?php } else { ?>
                            <div class="card">
                                <div class="card-body p-2">
                                    <p class="mb-0">No Western-region video listings have been posted in the last 14 days.</p>
                                </div>
                            </div>
                        <?php } ?>
                        ...
                        <?php } else { ?>
                            <div class="card">
                                <div class="card-body p-3">
                                    <p class="mb-0">No Western-region image listings have been posted in the last 14 days.</p>
                                </div>
                            </div>
                        <?php } ?>
                        <div class="listing-load-more-wrap">
