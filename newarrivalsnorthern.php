...
$northernListingsStmt = $conn->prepare(
    "SELECT l.*, u.businessname AS seller_businessname, u.profilepic AS seller_profilepic
     ...
     WHERE COALESCE(l.out_of_stock, 0) = 0
       ...
       AND LOWER(TRIM(COALESCE(l.region, ''))) = 'northern'
       AND l.created_at >= DATE_SUB(NOW(), INTERVAL 14 DAY)
     ORDER BY l.created_at DESC, l.listing_id DESC"
);

if ($northernListingsStmt) {
    $northernListingsStmt->execute();
    $northernListingsRes = $northernListingsStmt->get_result();
    while ($row = $northernListingsRes->fetch_assoc()) {
...
<head>
    ...
    <title>New Arrivals | Northern Uganda</title>
    <meta name="descrption" content="Browse the lates products, suppliers, businesses and services from Northern
    Uganda on JoMu and discover new business opportunities.">
    ...
    <main class="main-one">
        <div class="container-fluid">
            <div class="container mb-3 mt-3 newarrivals-container-one">
                <h4 style="font-weight: 700">New Arrivals - <span style="color: rgb(241, 90, 36);">Northern
                        Uganda</span>
                </h4>
                <hr>
                <p>Step into the vibrant wholesale marketplace of Northern Uganda, where businesses - suppliers and 
                    manufacturers from towns like<b style="color: rgb(241, 90, 36);"> Gulu, Lira, Arua, Kitgum</b> and beyond are
                        sharing their latest arrivals. From new products to exciting services, explore fresh listings tailored to your
                    business needs. <i><b>Be sure to check back regularly, there's always something new coming in from the North.</b></i>
                </p>
            </div>
                        ...
                        <?php } else { ?>
                            <div class="card">
                                <div class="card-body p-2">
                                    <p class="mb-0">No Northern-region video listings have been posted in the last 14 days.</p>
                                </div>
                            </div>
                        <?php } ?>
                        ...
                        <?php } else { ?>
                            <div class="card">
                                <div class="card-body p-3">
                                    <p class="mb-0">No Northern-region image listings have been posted in the last 14 days.</p>
                                </div>
                            </div>
                        <?php } ?>
                        <div class="listing-load-more-wrap">
